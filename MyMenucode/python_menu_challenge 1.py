...
while True:
    ...
    if menu_category.isdigit():
        ...
        menu_selection = input("Please enter the number of the item you would like to order or 'q' to quit:")
            
        if menu_selection.lower() == 'q':
                print("Thank you for visiting. Goodbye!")
                break
            
        #Add code to handle the quitting action 
        sys.exit()
        # Convert the menu selection to an integer
    elif menu_selection.isdigit() and int(menu_selection) in menu_items:
        selected_item = menu_items[int(menu_selection)]
        selected_quantity = input(f"How many {selected_item['Item name']} would you like? (Default is 1): ")
        ...
        print(f"{menu_selection} was not a valid item number of the menu options.Please try again.")
...
while True:
    ...
    for item in order_list:
        ...
        formatted_item_name = "{:<25}".format(str(item_name))
        formatted_price = "${:8.2f}".format(float(price))
        formatted_quantity = "{:<10}".format(int(quantity))
